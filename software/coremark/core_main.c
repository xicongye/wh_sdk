...
#if HAS_FLOAT
	ee_printf("Total time (secs): %6.3f\n",time_in_secs(total_time));
	if (time_in_secs(total_time) > 0)
		ee_printf("Iterations/Sec   : %6.3f\n",default_num_contexts*results[0].iterations/time_in_secs(total_time));
#else 
...
#if HAS_FLOAT
		if (known_id==3) {
			ee_printf("CoreMark 1.0 : %6.3f / %s %s",default_num_contexts*results[0].iterations/time_in_secs(total_time),COMPILER_VERSION,COMPILER_FLAGS);
